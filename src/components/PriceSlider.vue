...
<script>
  export default {
    ...
    methods: {
      findLargestDecimal(array) {
        // Initialize the largest decimal to the first element of the array.
        let largestDecimal = array[0];

        // Iterate over the array, starting at the second element.
        for (let i = 1; i < array.length; i++) {
          // If the current element is greater than the largest decimal, update the largest decimal.
          if (array[i] > largestDecimal) {
            largestDecimal = array[i];
          }
        }
        return largestDecimal;
      },
      findSmallestDecimal(array) {
        // Initialize the largest decimal to the first element of the array.
        let smallestDecimal = array[0];

        // Iterate over the array, starting at the second element.
        for (let i = 1; i < array.length; i++) {
          // If the current element is greater than the smallest decimal, update the smallest decimal.
          if (array[i] < smallestDecimal) {
            smallestDecimal = array[i];
          }
        }
